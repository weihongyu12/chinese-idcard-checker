...
describe('验证中国居民身份证', () => {
  ...
  it('获取省份（身份证号错误）', () => {
    const result = IDCardChecker.getProvince('160105194912310029');
  ...
  it('获取出生日期（身份证号错误）', () => {
    const result = IDCardChecker.getBirthDate('110105194902310026');
    expect(result).to.equal(null);
  });
  ...
  it('获取性别（男）', () => {
    const result = IDCardChecker.getGender('350724199111282910');
    expect(result).to.equal('男');
  });

  it('获取性别（身份证号错误）', () => {
    const result = IDCardChecker.getGender('110105194912310021');
    expect(result).to.equal(null);
  });
});
